...
public class SougouScelFileProcessing extends FileProcessing {
    ...
    public void parseFile(String filePath, boolean isAppend) {
        ...
        if (file.isDirectory()) {
            ...
            for (int i = 0; i < items.length; i++) {
                ...
                if (targetDir == null) {
                    parseFile(items[i].getAbsolutePath(),
                            items[i].getAbsolutePath().replace(".scel", ".txt"),
                            isAppend);
                } else {
                    parseFile(items[i].getAbsolutePath(),
                            targetDir + "/" +
                                    items[i].getName().replace(".scel", ".txt"),
                            isAppend);
                }
            }
        } else {
    ...
    private void writeToTargetFile(List<SougouScelModel> models, String targetFilePath, boolean isAppend)
        ...
        int count = 0;
        Map<String, List<String>> words = new HashMap<>(); // 词
        for (int k = 0; k < models.size(); k++) {
            words.putAll(models.get(k).getWordMap());
        }
